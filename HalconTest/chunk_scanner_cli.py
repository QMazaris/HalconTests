                ...
                if current_title is not None and (description_lines or code_lines):
                    ...
                    if max_code_lines and len(code_lines) > max_code_lines > 0:
                        for part_idx in range(0, len(code_lines), max_code_lines):
                            ...
                            chunks.append({
                                "title": f"{current_title.strip()} (part {part_idx // max_code_lines + 1})",
...
def main() -> None:
    ...
    sqlite_db_path = "halcon_code_examples.db"
    ...
    if verbose:
        ...
        print(f"Processed {file_count} files – extracted {chunk_total} chunks.")
